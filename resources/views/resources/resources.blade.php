            ...
            <div class="col-md-3">
                <div class="nav-tabs-custom">
                    <div class="tab-content">
                        @foreach($room as $risorsa)
                            <h4>Location:<strong>{{$risorsa->location}}</strong></h4>
                          <h4>Risorsa: <strong>{{$risorsa->name}}</strong></h4>
                            <br />
                            <br />
                            <br />
                        <form action="{{ route('bookingshowroom')}}" method="POST">
                            {{ csrf_field() }}
                            <div class="row">
                                <div class="col-md-12">
                                    <!-- Date and time range -->
                                ...
                                <div class="col-md-6" style="display: none">
                                    <div class="input-group">
                                        <label>{{ __('Room Id') }}</label>
                                        <input name="roomId" id="roomId" value="{{$risorsa->id}}">
                                    </div>
                                        <div class="input-group">
                                            <label>{{ __('Room Name') }}</label>
                                                <input name="roomName" id="roomName" value="{{$risorsa->name}}">
                                        </div>
                                        <div class="input-group">
                                            <label>{{ __('Location') }}</label>
                                            <input name="location" id="location" value="{{$risorsa->location}}">
                                        </div>
                                </div>
                            </div>
